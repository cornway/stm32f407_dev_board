...
uint8_t enc_RCR(enc_device_t *dev, enc_register_t reg) {
	ensure_register_accessible(dev, (enc_register_t)reg);
...
uint16_t enc_RCR16(enc_device_t *dev, enc_register_t reg) {
	return (enc_RCR(dev, (enc_register_t)(reg|1)) << 8) | enc_RCR(dev, (enc_register_t)(reg&~1));
}
...
{
	uint16_t state;
	state = enc_MII_read(dev, (enc_register_t)ENC_PHLCON);
...
void enc_transmit_pbuf(enc_device_t *dev, struct pbuf *buf)
{
	...
	transmit_start(dev);
	while(1) {
		transmit_partial(dev, (uint8_t *)buf->payload, buf->len);
...
int enc_read_received_pbuf(enc_device_t *dev, struct pbuf **buf)
{
	...
	*buf = pbuf_alloc(PBUF_RAW, length, PBUF_RAM);
	if (*buf == NULL)
		...
		enc_RBM(dev, (uint8_t *)(*buf)->payload, ENC_READLOCATION_ANY, length);
